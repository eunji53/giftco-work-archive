...
class LabelingToolApp(tk.Tk):
    ...
    def render(self):
        ...
        for i, row in enumerate(st["tokens"]):
            ...
            # --- 추가: 클릭 시 수정입력에 단어 복사 ---
            # def on_word_click(event, token=row["token"]):
            #     self.edit_var.set(token)
            #     self.edit_entry.configure(foreground="#000")
            # word_lbl.bind("<Button-1>", on_word_click)

            def on_check_change(idx):
                # idx: 체크된 단어의 index
                checked_rows = [i for i, w in self.row_widgets.items() if w["chk_var"].get()]
                if checked_rows:
                    # 마지막 체크된 단어를 입력
                    last_idx = checked_rows[-1]
                    token = self.state[self.idx]["tokens"][last_idx]["token"]
                    self.edit_var.set(token)
                    self.edit_entry.focus_set()

            word_lbl.bind("<Button-1>", on_check_change)
