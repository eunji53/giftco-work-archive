...
# 1. 파일 설정
# =========================
# 상품 파일은 프로그램 폴더 기준 data/ 에 고정으로 둡니다 (실제 파일: partner_goods_full_join.xlsx).
DATA_DIR = "data"
JOIN_FILE = f"{DATA_DIR}/partner_goods_full_join.xlsx"

# 공급사 파일은 실행 시 사용자가 직접 선택합니다. 마지막 선택 경로는 config에 저장됩니다.
# (v6) 구글시트 설정도 이 파일에 함께 저장하므로 google_service_account.json과 같은 config/ 폴더에 둡니다.
CONFIG_FILE = "config/supplier_product_viewer_config.json"

# 연락상태는 공급사 파일을 직접 수정하지 않고 별도 파일에 저장합니다.
CONTACT_STATUS_FILE = "공급사_연락상태.xlsx"

# Google Sheets 연동 설정 (v6 신규 추가분).
# 연락상태 공유를 위한 값이며, config 파일(supplier_product_viewer_config.json)의
# google_service_account_path / google_sheet_id / google_worksheet_name 키가 있으면 그 값을 우선 사용합니다.
GOOGLE_SERVICE_ACCOUNT_FILE_DEFAULT = "config/google_service_account.json"
GOOGLE_SHEET_ID_DEFAULT = ""
GOOGLE_WORKSHEET_NAME_DEFAULT = ""
GOOGLE_SHEET_SCOPES = [
...
PRODUCT_REGISTRATION_OPTIONS = ["전체", "미등록", "등록중", "등록완료"]
...
# 왼쪽 공급사 목록에 표시할 컬럼
SUPPLIER_LIST_COLUMNS = ["등록", "업체명", "담당자", "진열상품수", "연락일자"]
...
class SupplierProductViewer(tk.Tk):
    def __init__(self):
        ...
        # Google Sheets 연동 설정 (v6 신규 추가분). load_config()에서 config 파일 값으로 덮어씁니다.
        self.google_sa_path = self.app_dir / GOOGLE_SERVICE_ACCOUNT_FILE_DEFAULT
        self.google_sheet_id = GOOGLE_SHEET_ID_DEFAULT
        self.google_worksheet_name = GOOGLE_WORKSHEET_NAME_DEFAULT
        self._gs_worksheet = None
    ...
    def _setup_style(self):
        ...
        style.configure("TButton", font=base_font, padding=(6, 0))
        style.configure("Compact.TButton", font=base_font, padding=(6, 0))
        style.configure("TLabelframe.Label", font=("맑은 고딕", 10, "bold"))

        # 연락상태 버튼 — 공급사 목록 행 배경색과 동일
        for name, color in [
            ("Done",  "#E2F0D9"),
            ("Retry", "#FFF2CC"),
            ("Fail",  "#E7E6E6"),
            ...
            style.configure(f"Contact.{name}.TButton", font=base_font, padding=(6, 0), background=color)
            style.map(f"Contact.{name}.TButton",
                      background=[("active", color), ("pressed", color), ("!disabled", color)])
    ...
    def _build_layout(self):
        ...
        # 검색 1행: 키워드 검색
        search_row1 = ttk.Frame(supplier_box)
        search_row1.pack(fill=tk.X, pady=(0, 3))
        ttk.Label(search_row1, text="검색").pack(side=tk.LEFT)
        self.supplier_search_var = tk.StringVar()
        self.supplier_search_var.trace_add("write", lambda *args: self.refresh_supplier_list())
        ttk.Entry(search_row1, textvariable=self.supplier_search_var).pack(side=tk.LEFT, fill=tk.X, expand=True, padx=(4, 0))

        # 검색 2행: 담당자 · 연락 · 등록 필터
        search_row2 = ttk.Frame(supplier_box)
        search_row2.pack(fill=tk.X, pady=(0, 5))
        ttk.Label(search_row2, text="담당자").pack(side=tk.LEFT)
        self.owner_filter_var = tk.StringVar(value="전체")
        self.owner_filter_combo = ttk.Combobox(
            search_row2, textvariable=self.owner_filter_var,
            values=["전체"], state="readonly", width=7,
        )
        self.owner_filter_combo.pack(side=tk.LEFT, padx=(3, 0))
        self.owner_filter_combo.bind("<<ComboboxSelected>>", lambda event: self.refresh_supplier_list())
        ttk.Label(search_row2, text="연락").pack(side=tk.LEFT, padx=(8, 3))
        self.contact_filter_var = tk.StringVar(value="전체")
        self.contact_filter_combo = ttk.Combobox(
            search_row2, textvariable=self.contact_filter_var,
            values=CONTACT_STATUS_OPTIONS, state="readonly", width=8,
        )
        ...
        ttk.Label(search_row2, text="등록").pack(side=tk.LEFT, padx=(8, 3))
        self.registration_filter_var = tk.StringVar(value="전체")
        self.registration_filter_combo = ttk.Combobox(
            search_row2, textvariable=self.registration_filter_var,
            values=PRODUCT_REGISTRATION_OPTIONS, state="readonly", width=7,
        )
        self.registration_filter_combo.pack(side=tk.LEFT)
        self.registration_filter_combo.bind("<<ComboboxSelected>>", lambda event: self.refresh_supplier_list())
        ...
        contact_button_frame.pack(fill=tk.X, pady=(0, 2))
        ttk.Label(contact_button_frame, text="연락상태:", width=7).pack(side=tk.LEFT)
        ttk.Button(contact_button_frame, text="연락완료", style="Contact.Done.TButton", command=lambda: self.update_contact_status("연락완료")).pack(side=tk.LEFT, padx=(0, 3))
        ttk.Button(contact_button_frame, text="재연락필요", style="Contact.Retry.TButton", command=lambda: self.update_contact_status("재연락필요")).pack(side=tk.LEFT, padx=(0, 3))
        ttk.Button(contact_button_frame, text="연락불가", style="Contact.Fail.TButton", command=lambda: self.update_contact_status("연락불가")).pack(side=tk.LEFT, padx=(0, 3))
        ttk.Button(contact_button_frame, text="미연락", style="Contact.None.TButton", command=lambda: self.update_contact_status("미연락")).pack(side=tk.LEFT)

        reg_button_frame = ttk.Frame(info_box)
        reg_button_frame.pack(fill=tk.X, pady=(0, 2))
        ttk.Label(reg_button_frame, text="등록상태:", width=7).pack(side=tk.LEFT)
        ttk.Button(reg_button_frame, text="● 등록완료", style="Compact.TButton", command=lambda: self.update_product_status("등록완료")).pack(side=tk.LEFT, padx=(0, 3))
        ttk.Button(reg_button_frame, text="◑ 등록중", style="Compact.TButton", command=lambda: self.update_product_status("등록중")).pack(side=tk.LEFT, padx=(0, 3))
        ttk.Button(reg_button_frame, text="○ 미등록", style="Compact.TButton", command=lambda: self.update_product_status("미등록")).pack(side=tk.LEFT)

        manager_frame = ttk.Frame(info_box)
        manager_frame.pack(fill=tk.X, pady=(0, 6))
        ttk.Label(manager_frame, text="담당자:", width=7).pack(side=tk.LEFT)
        self.manager_entry_var = tk.StringVar()
        ttk.Entry(manager_frame, textvariable=self.manager_entry_var, width=16).pack(side=tk.LEFT, padx=(0, 4))
        ttk.Button(manager_frame, text="저장", style="Compact.TButton", command=self.update_manager).pack(side=tk.LEFT)
        ...
        # 분리대(sash) 드래그 후 마우스를 떼면 현재 크기를 자동 저장
        for _pw in [self.main_paned, self.right_paned, self.top_right_paned, self.bottom_paned]:
            _pw.bind("<ButtonRelease-1>", lambda e: self.after(100, self.save_config))

    def force_layout(self):
        """초기 화면 비율을 강제로 조정합니다. 저장된 값이 있으면 우선 사용합니다."""
        try:
            self.update_idletasks()
            w = max(self.winfo_width(), 1300)
            h = max(self.winfo_height(), 800)
            s = getattr(self, "saved_sash", {})

            left_w   = s.get("left")      or 510
            top_h    = s.get("right_top") or 400
            top_rw   = s.get("top_right") or 890
            bottom_w = s.get("bottom")    or 580

            self.main_paned.sashpos(0, left_w)
            self.right_paned.sashpos(0, top_h)
            self.top_right_paned.sashpos(0, top_rw)
            self.bottom_paned.sashpos(0, bottom_w)

            self.after(60, lambda: self._apply_sash(left_w, top_h, top_rw, bottom_w))
        except Exception:
            pass

    def _apply_sash(self, left_w: int, top_h: int, top_rw: int, bottom_w: int):
        """렌더링 완료 후 sashpos 재적용."""
        try:
            self.update_idletasks()
            self.main_paned.sashpos(0, left_w)
            self.right_paned.sashpos(0, top_h)
    ...
    def _config_tree(self, tree: ttk.Treeview, widths: dict[str, int]):
        for col, width in widths.items():
            ...
            if col in {"상품수", "연락", "연락여부", "연락일자", "진열상품수", "등록상태", "등록", "진열", "노출", "담당자"}:
                anchor = tk.CENTER
    ...
    def load_config(self):
        """공급사 파일 경로와 저장된 레이아웃 크기를 불러옵니다."""
        self.saved_sash: dict = {}
        try:
            if self.config_path.exists():
                ...
                self.saved_sash = config.get("sash_positions", {})
                # Google Sheets 연동 설정 (v6 신규 추가분, 기존 키는 그대로 둠)
                sa_path_cfg = clean_text(config.get("google_service_account_path", ""))
                if sa_path_cfg:
                    self.google_sa_path = self.app_dir / sa_path_cfg
                self.google_sheet_id = clean_text(config.get("google_sheet_id", "")) or self.google_sheet_id
                self.google_worksheet_name = clean_text(config.get("google_worksheet_name", "")) or self.google_worksheet_name
        except Exception:
            self.supplier_path = None

    def save_config(self):
        """공급사 파일 경로와 현재 패널 크기를 저장합니다."""
        try:
            sash: dict = {}
            try:
                sash["left"]      = self.main_paned.sashpos(0)
                sash["right_top"] = self.right_paned.sashpos(0)
                sash["top_right"] = self.top_right_paned.sashpos(0)
                sash["bottom"]    = self.bottom_paned.sashpos(0)
            except Exception:
                pass
            config = {
                ...
                "sash_positions": sash,
                # Google Sheets 연동 설정 (v6 신규 추가분)
                "google_service_account_path": GOOGLE_SERVICE_ACCOUNT_FILE_DEFAULT,
    ...
    def _describe_gsheet_error(self, e: Exception) -> str:
        """예외를 사용자가 원인을 알 수 있는 문구로 바꿉니다. private_key 등 JSON 원문은 노출하지 않습니다."""
        if gspread is not None:
            if isinstance(e, gspread.exceptions.SpreadsheetNotFound):
                return "스프레드시트 ID가 잘못되었거나 서비스 계정에 공유되지 않았습니다."
            if isinstance(e, gspread.exceptions.WorksheetNotFound):
                return f"워크시트 '{self.google_worksheet_name}'를 찾을 수 없습니다. 이름을 확인하세요."
            if isinstance(e, gspread.exceptions.APIError):
                return f"구글 API 오류입니다. 서비스 계정 공유 권한/네트워크를 확인하세요. ({e})"
        return str(e)[:300]

    def _get_gsheet_worksheet(self):
        """연락상태 공유용 구글시트 워크시트를 반환합니다. 연결에 성공하면 재사용을 위해 캐시합니다."""
        if self._gs_worksheet is not None:
            return self._gs_worksheet
        if gspread is None or GoogleCredentials is None:
            raise RuntimeError("gspread/google-auth가 설치되어 있지 않습니다. pip install gspread google-auth 후 다시 실행하세요.")
        if not self.google_sa_path.exists():
            raise RuntimeError(f"구글 서비스 계정 키 파일을 찾을 수 없습니다: {self.google_sa_path}")
        if not self.google_sheet_id:
            raise RuntimeError(f"구글시트 ID가 설정되어 있지 않습니다. {self.config_path} 파일의 google_sheet_id 값을 입력하세요.")
        if not self.google_worksheet_name:
            raise RuntimeError(f"워크시트 이름이 설정되어 있지 않습니다. {self.config_path} 파일의 google_worksheet_name 값을 입력하세요.")
        creds = GoogleCredentials.from_service_account_file(str(self.google_sa_path), scopes=GOOGLE_SHEET_SCOPES)
        client = gspread.authorize(creds)
        spreadsheet = client.open_by_key(self.google_sheet_id)
        self._gs_worksheet = spreadsheet.worksheet(self.google_worksheet_name)
        return self._gs_worksheet

    def _read_or_create_contact_status(self) -> "pd.DataFrame":
        """연락상태를 구글시트에서 읽습니다 (v6: 기존 공급사_연락상태.xlsx 읽기를 대체).
        연동에 실패하면 로컬 파일로 대체하지 않고, 원인을 알리고 빈 구조로 시작합니다."""
        try:
            ws = self._get_gsheet_worksheet()
            # numericise_ignore=['all']: 업체코드처럼 숫자로 보이는 값이 gspread에 의해
            # 자동으로 int로 바뀌면 norm_key() 매칭이 깨지므로 전부 문자열로 읽습니다.
            records = ws.get_all_records(numericise_ignore=["all"])
            df = pd.DataFrame(records)
        except Exception as e:
            messagebox.showerror(
                "구글시트 연결 실패",
                "연락상태 데이터를 구글시트에서 불러오지 못했습니다.\n"
                "로컬 파일로 대체되지 않으며, 연결이 복구되기 전까지 화면은 빈 값으로 표시됩니다.\n\n"
                f"원인: {self._describe_gsheet_error(e)}"
            )
    ...
    def _backup_contact_status_locally(self):
        """혹시 모를 상황에 대비해 연락상태 전체를 data/ 폴더에 날짜별로 백업합니다.
        구글시트가 유일한 저장소이며, 이 백업 파일은 프로그램이 읽어들이지 않는
        참고/비상용 스냅샷입니다 (같은 날 여러 번 저장하면 그날 파일에 덮어써집니다)."""
        try:
            backup_dir = self.join_path.parent
            backup_dir.mkdir(parents=True, exist_ok=True)
            date_str = datetime.now().strftime("%Y%m%d")
            base_name = Path(CONTACT_STATUS_FILE).stem
            backup_path = backup_dir / f"{base_name}_{date_str}.xlsx"

            out = self.contact_status_df.copy()
            for col in CONTACT_STATUS_COLUMNS:
                if col not in out.columns:
                    out[col] = ""
            out[CONTACT_STATUS_COLUMNS].to_excel(backup_path, index=False)
        except Exception:
            pass  # 백업 실패는 조용히 무시하고 핵심 저장 흐름(구글시트)에 영향을 주지 않습니다.

    def save_contact_status_file(self):
        """연락상태를 구글시트에 저장합니다 (v6: 기존 로컬 공급사_연락상태.xlsx 저장을 대체).
        전체 시트를 덮어쓰지 않고, 현재 선택된 공급사(self.selected_supplier_code/name) 행만
        구글시트를 다시 조회해 찾은 뒤 그 행만 수정하거나(없으면) 새로 추가합니다.
        저장 성공/실패와 무관하게 data/ 폴더에 날짜별 로컬 백업을 남깁니다."""
        try:
            ...
            if not hit.any():
                return
            row = self.contact_status_df[hit].iloc[-1]
            row_values = {col: clean_text(row.get(col, "")) for col in CONTACT_STATUS_COLUMNS}
            ...
            if all_values:
                ...
                header = CONTACT_STATUS_COLUMNS
                ws.append_row(header, value_input_option="USER_ENTERED")
                all_values = [header]

            code_idx = header.index("업체코드") if "업체코드" in header else 0
            name_idx = header.index("업체명") if "업체명" in header else 1

            # 저장 직전에 구글시트를 다시 조회해 최신 행 위치를 찾습니다 (행 번호를 캐시하지 않음).
            target_row_num = None
            for i, r in enumerate(all_values[1:], start=2):
                r_code = r[code_idx] if code_idx < len(r) else ""
                r_name = r[name_idx] if name_idx < len(r) else ""
                if norm_key(r_code) + "|" + norm_key(r_name) == supplier_key:
                    target_row_num = i
                    break

            values_row = [row_values.get(col, "") for col in header]
            if target_row_num is None:
                ws.append_row(values_row, value_input_option="USER_ENTERED")
            else:
                ws.update(range_name=f"A{target_row_num}", values=[values_row])
        except Exception as e:
            messagebox.showerror(
                "구글시트 저장 실패",
                f"연락상태를 구글시트에 저장하지 못했습니다. 다른 사람과 공유되는 저장은 되지 않았습니다.\n\n"
                f"원인: {self._describe_gsheet_error(e)}"
            )

        self._backup_contact_status_locally()
    ...
    def _prepare_supplier_list(self):
        ...
        # 진열 상태인 상품 수만 별도 집계
        display_goods = self.goods_df[self.goods_df["진열_표시"] == "진열"]
        if not display_goods.empty:
            disp_pair = display_goods.groupby("공급사_KEY", dropna=False).size().reset_index(name="진열상품수_pair")
            disp_code = display_goods.groupby("업체코드_KEY", dropna=False).size().reset_index(name="진열상품수_code")
        else:
            disp_pair = pd.DataFrame(columns=["공급사_KEY", "진열상품수_pair"])
            disp_code = pd.DataFrame(columns=["업체코드_KEY", "진열상품수_code"])
        supplier = supplier.merge(disp_pair, on="공급사_KEY", how="left")
        supplier = supplier.merge(disp_code, on="업체코드_KEY", how="left")
        supplier["진열상품수"] = supplier["진열상품수_pair"].fillna(supplier["진열상품수_code"]).fillna(0).astype(int)
        supplier = supplier.drop(columns=[c for c in ["진열상품수_pair", "진열상품수_code"] if c in supplier.columns])

        # 연락상태 기본 컬럼
        for col in ["담당자", "연락여부", "연락일자", "연락수단", "연락결과", "재연락예정일", "메모", "등록상태", "최종수정일시"]:
        ...
        supplier["등록"] = supplier["등록상태"].map(clean_text).map(self._registration_mark)
    ...
    def _refresh_filter_values(self):
        ...
        if hasattr(self, "registration_filter_var"):
            self.registration_filter_var.set("전체")
        self.owner_filter_var.set("전체")
        self.status_filter_var.set("전체")
        self._update_owner_filter_values()

    # ---------------- Supplier list/detail ----------------
    def _update_owner_filter_values(self):
        """담당자 필터 드롭다운의 선택 목록을 현재 공급사 데이터 기준으로 갱신합니다."""
        if not hasattr(self, "owner_filter_combo"):
            return
        if self.supplier_list_df.empty or "담당자" not in self.supplier_list_df.columns:
            self.owner_filter_combo["values"] = ["전체"]
            return
        owners = sorted(
            v for v in self.supplier_list_df["담당자"].map(clean_text).unique() if v
        )
        self.owner_filter_combo["values"] = ["전체"] + owners

    def update_product_status(self, status: str):
        """선택한 공급사의 상품 등록상태를 저장하고 화면을 갱신합니다."""
        ...
        if hit.any():
            ...
            new_row = {
                "업체코드": supplier_code, "업체명": supplier_name,
                "담당자": clean_text(row.get("담당자", "")),
                "연락여부": "", "연락일자": "", "연락수단": "", "연락결과": "",
                "재연락예정일": "", "메모": clean_text(row.get("메모", "")),
                "등록상태": "", "최종수정일시": "",
                "업체코드_KEY": code_key, "업체명_KEY": name_key, "공급사_KEY": supplier_key,
            }
        ...
        self.contact_status_df.loc[idx, "등록상태"] = status
        ...
        self.status_var.set(f"{supplier_name} → 등록상태: {status} 처리 완료")

    def update_manager(self):
        """선택한 공급사의 담당자를 저장하고 화면을 갱신합니다."""
        row = self._selected_supplier_row()
        if row is None:
            messagebox.showinfo("안내", "공급사를 먼저 선택하세요.")
            return

        manager_name = clean_text(self.manager_entry_var.get()) if hasattr(self, "manager_entry_var") else ""
        supplier_code = clean_text(row.get("업체코드", ""))
        supplier_name = clean_text(row.get("업체명", ""))
        code_key = norm_key(supplier_code)
        name_key = norm_key(supplier_name)
        ...
        if hit.any():
            ...
            new_row = {
                "업체코드": supplier_code, "업체명": supplier_name,
                "담당자": "", "연락여부": "", "연락일자": "", "연락수단": "", "연락결과": "",
                "재연락예정일": "", "메모": "", "등록상태": "", "최종수정일시": "",
                "업체코드_KEY": code_key, "업체명_KEY": name_key, "공급사_KEY": supplier_key,
            }
            self.contact_status_df = pd.concat([self.contact_status_df, pd.DataFrame([new_row])], ignore_index=True)
            idx = self.contact_status_df.index[-1]
        ...
        self.contact_status_df.loc[idx, "업체코드"] = supplier_code
        self.contact_status_df.loc[idx, "업체명"] = supplier_name
        self.contact_status_df.loc[idx, "담당자"] = manager_name
        self.contact_status_df.loc[idx, "최종수정일시"] = now
        ...
        self.refresh_supplier_list()
        self.show_supplier_detail()
        self.status_var.set(f"{supplier_name} → 담당자: {manager_name if manager_name else '(없음)'} 저장 완료")

    def _registration_mark(self, status: str) -> str:
    ...
    def refresh_supplier_list(self):
        ...
        if owner and owner != "전체":
            df = df[df["담당자"].astype(str).eq(owner)]
        ...
        if contact_filter and contact_filter != "전체":
            df = df[df["연락여부"].astype(str).eq(contact_filter)]
        registration_filter = self.registration_filter_var.get() if hasattr(self, "registration_filter_var") else "전체"
        if registration_filter and registration_filter != "전체":
            df = df[df["등록상태"].astype(str).eq(registration_filter)]

        for idx, row in df.iterrows():
            values = []
            for col in SUPPLIER_LIST_COLUMNS:
                if col in {"상품수", "진열상품수"}:
                    values.append(f"{safe_int(row.get(col, 0)):,}")
                else:
    ...
    def on_supplier_select(self, event=None):
        ...
        if hasattr(self, "manager_entry_var"):
            self.manager_entry_var.set(clean_text(row.get("담당자", "")))
    ...
    def show_supplier_detail(self):
        ...
        self.info_text.insert(tk.END, "      ")
        display_count = safe_int(row.get("진열상품수", 0))
        self.info_text.insert(tk.END, f"진열상품수 : {display_count:,}", "summary")
        self.info_text.insert(tk.END, "      ")
        self.info_text.insert(tk.END, f"카테고리수 : {category_count:,}", "summary")
        self.info_text.insert(tk.END, "\n")

        reg_status = clean_text(row.get("등록상태", "")) or "미등록"
        manager_val = clean_text(row.get("담당자", "")) or "-"
        contact_date_text = f" ({contact_date})" if contact_date else ""
        self.info_text.insert(tk.END, "연락상태 : ", "label")
        self.info_text.insert(tk.END, f"{contact_status}{contact_date_text}   ", "value")
        self.info_text.insert(tk.END, "등록상태 : ", "label")
        self.info_text.insert(tk.END, f"{reg_status}   ", "value")
        self.info_text.insert(tk.END, "담당자 : ", "label")
        self.info_text.insert(tk.END, manager_val, "value")
        self.info_text.insert(tk.END, "\n\n")
    ...
    def show_major_summary(self):
        ...
        if gdf.empty:
            return
        gdf_display = gdf[gdf["진열_표시"] == "진열"]
        summary = gdf_display["대분류_표시"].map(clean_text).replace("", "미분류").value_counts().reset_index()
    ...
    def show_category_summary(self):
        ...
        if gdf.empty:
            return
        gdf_display = gdf[gdf["진열_표시"] == "진열"]
        vc = gdf_display["카테고리_표시"].map(clean_text).replace("", "미분류").value_counts().reset_index()
        vc.columns = ["카테고리", "개수"]
    ...
    def export_assignment_template(self):
        ...
        try:
            cols = [
                "연락", "연락여부", "연락일자", "연락수단", "연락결과", "재연락예정일", "담당자", "메모", "등록상태",
                "업체코드", "업체명", "진열상품수", "상품수", "조인상품수", "카테고리수",
            ...
            messagebox.showinfo(
                "완료",
                f"연락상태 목록을 저장했습니다 (스냅샷, 참고용).\n\n{path}\n\n"
                f"실제 연락상태 누적 데이터는 로컬 파일이 아니라 구글시트에 자동 저장됩니다."
            )
        except Exception as e:
            messagebox.showerror("오류", f"저장 실패: {e}")
