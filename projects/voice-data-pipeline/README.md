...
├─ analyze_inbound_calls.py                  # 완성본: 위와 같은 로직의 전체 배치용 스크립트 (야간 실행)
...
│  ├─ inbound/    # 01/analyze_inbound_calls.py의 수신 분류결과 (csv/json/checkpoint) — gitignore 대상
│  └─ outbound/   # 발신통화 정리 시 사용 예정 (현재 비어 있음)
└─ output/
   ├─ inbound/    # 02의 결과 이미지 11개 — gitignore 대상
...
4. **소량 테스트**: `01_analyze_inbound_calls.ipynb` 실행 → `data/inbound/`에 결과 생성. `TEST_N`(개수) 또는 `TEST_DATE`(예: `"20260601"`)로 범위를 좁혀서 확인
5. **전체 배치**: 테스트가 끝나면 `python analyze_inbound_calls.py`로 전체 실행 (야간 실행 권장, 체크포인트로 중단 후 재개 가능). 01 노트북과 같은 `data/inbound/` 결과 파일을 이어서 씁니다
6. `02_visualize_inbound_call_analysis.ipynb` 실행 → `output/inbound/`에 이미지 11개 생성
